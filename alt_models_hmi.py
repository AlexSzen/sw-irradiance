...
class AugResnet2(nn.Module):
    def __init__(self, resnet, outSize, dropout):
        ...
        self.resnet_18.conv1 = nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        if dropout:
            self.fc = nn.Sequential(
                    nn.Dropout(0.5),
                    nn.Linear(512+3, outSize)
                )
        else:
            self.fc = nn.Linear(512+3, outSize)
...
class ChoppedAlexnetBN(nn.Module):
    def getLayers(self, numLayers):
        ...
        layers = [nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2), nn.BatchNorm2d(64), nn.ReLU(inplace=True), ]
        if numLayers == 1:
            return (layers, 64)
...
class ChoppedAlexnetSpatial(nn.Module):
    ...
    def getLayers(self, numLayers):
        """Returns a list of layers + the feature size coming out"""
        layers = [nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2), nn.ReLU(inplace=True), ]
        if numLayers == 1:
            return (layers, 64, 55)
...
class ChoppedAlexnet64(nn.Module):
    def getLayers(self, numLayers):
        """Returns a list of layers + the feature size coming out"""
        layers = [nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2), nn.LeakyReLU(inplace=True), ]
        if numLayers == 1:
            return (layers, 64)
...
class ChoppedVGG(nn.Module):
    def getLayers(self, numLayers):
        """Returns a list of layers + the feature size coming out"""
        layers = [nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(64), 
