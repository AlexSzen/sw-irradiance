...
if __name__ == "__main__":
    ...
    for i in range(len(lines)):
        ...
        ax5.hist2d(np.log10(eve[:, i]), np.log10(eve[:, i] * (1.0 - error[:, i])), range=hrange, bins=[50, 50],
                           norm=matplotlib.colors.LogNorm(),cmap=plt.cm.magma_r, cmin=1)
        ax5.plot([r1, r2], [r1, r2])
        ax5.set_xlabel('Observed  : Log10 [Irr/(W m^-2)]',fontsize=24)
        ax5.set_ylabel('Predicted : Log10 [Irr/(W m^-2)]',fontsize=24)
        ax5.set_title('{0}: {1} log T = {2:.2f}'.format(modelname, name[i], logT[i]),fontsize=30)
