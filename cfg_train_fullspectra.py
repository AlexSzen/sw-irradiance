...
from SW_Dataset_fullspectra import *
...
if __name__ == "__main__":
    ...
    for cfgi,cfgPath in enumerate(cfgs):
        ...
        if cfg['arch'] == "resnet_18":
            ...
            if cfg['dropout']:
                sw_net.fc = nn.Sequential(
                    nn.Dropout(0.5),
                    nn.Linear(num_ftrs, num_eve_channels)
                )
            else:
                sw_net.fc = nn.Linear(num_ftrs, num_eve_channels)
                
        elif cfg['arch'] == "avg_mlp":
            sw_net = alt_models.AvgMLP(9,1024,num_eve_channels, cfg['dropout'])
        elif cfg['arch'].startswith("anet"):
            layerCount = int(cfg['arch'].split("_")[1])
            if (len(cfg['arch'].split("_")) == 2): ###name is anet_numlayers
                sw_net = alt_models.ChoppedAlexnet(layerCount,num_eve_channels,cfg['dropout'])
            elif (len(cfg['arch'].split("_")) == 3): ### name is anet_numlayers_bn
                sw_net = alt_models.ChoppedAlexnetBN(layerCount,num_eve_channels,cfg['dropout'])
            else:
                raise ValueError('Arch not defined')
        elif cfg['arch'].startswith("vgg"):
            layerCount = int(cfg['arch'].split("_")[1])
            sw_net = alt_models.ChoppedVGG(layerCount,num_eve_channels,cfg['dropout'])
        else:
        ...
        ### Some inputs
        EVE_path = '/data/NASAFDL2018/SpaceWeather/Team1-Meng/EVE/np/eve_full_spectra/2011_eve_full_spectra.npy'
        ...
        aia_mean = np.load("%s/aia_sqrt_mean_fullspectra.npy" % data_root)
        aia_std = np.load("%s/aia_sqrt_std_fullspectra.npy" % data_root)
