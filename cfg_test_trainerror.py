     ...
     #   if cfg['arch'] == "resnet_18":
      ...
      #  elif cfg['arch'] == "avg_mlp":
       ...
       # else:
        ...
        ### Some inputs
        EVE_path = '/fastdata2/FDL/trainSetups/2011p4_new_fp16/irradiance_6m.npy'

        #now this should saturate the gpus
        #data_root = "/run/shm/AIA_256_20112014/"
        data_root = "/fastdata2/FDL/trainSetups/2011p4_new_fp16/"
        ...
        sw_datasets = {x: SW_Dataset(EVE_path, data_root, data_root, resolution, cfg['eve_transform'], cfg['eve_sigmoid'], split = x, AIA_transform = aia_transform, crop = crop, flip = flip, crop_res = crop_res, zscore = zscore) for x in ['train']}
        sw_dataloaders = {x: torch.utils.data.DataLoader(sw_datasets[x], batch_size = batch_size, shuffle = False, num_workers=8) for x in ['train']}
        dataset_sizes = {x: len(sw_datasets[x]) for x in ['train']}

        DS = sw_datasets['train']
        prediction = test_model(sw_net, sw_dataloaders['train'])
        ...
        absError = np.nanmean(absErrorPointwise,axis=0)
