    ...
    for cfgi,cfgPath in enumerate(cfgs):
        ...
        if cfg['arch'] == "resnet_18":
            ...
            if cfg['dropout']:
                sw_net.fc = nn.Sequential(
                    nn.Dropout(0.5),
                    nn.Conv2d(num_ftrs, 14, kernel_size=(1, 1), stride=(1, 1))
                )
            else:
                sw_net.fc = nn.Conv2d(num_ftrs, 14, kernel_size=(1, 1), stride=(1, 1))  
                
        elif cfg['arch'] == "avg_mlp":
            sw_net = alt_models.AvgMLP(9,1024,14, cfg['dropout'])
        elif cfg['arch'] == 'augresnet':
            sw_net = alt_models.AugResnet2(models.resnet18(pretrained=False), 14, cfg['dropout'])
        elif cfg['arch'] == 'augCNN':
            sw_net = alt_models.AugmentedCNN()
        elif cfg['arch'] == 'avgLinear':
            sw_net = alt_models.AvgLinearMap(9,14)
        elif cfg['arch'].startswith("anet64"):
            layerCount = int(cfg['arch'].split("_")[1])
            sw_net = alt_models_hmi.ChoppedAlexnet64(layerCount,14,cfg['dropout'])
            ...
            if (len(cfg['arch'].split("_")) == 2): ###name is anet_numlayers
                ...
                sw_net = alt_models_hmi.ChoppedAlexnetBN(layerCount,14,cfg['dropout'])
            else:
        ...
        data_root = "/fastdata/FDL/trainSetups/HMI_2011/"
